package cn.wulin.nio.socket.server;
	...
	public void init() throws IOException{
		...
		while(selector.select()>0){
			...
			for (SelectionKey sk : selector.selectedKeys()) {
				//从selector上的已经选择key集中删除正在处理的SelectionKey
				selector.selectedKeys().remove(sk);
				//如果sk对应的通道包含客户端的连接请求
				if(sk.isAcceptable()){
					//调用accetp方法接受连接,产生服务器端对应的SocketChannel
				...
				//如果sk对应的通道有数据需要读取
				if(sk.isReadable()){
					//读取该SelectionKey对应的Channel,该Channel中有可读的数据
