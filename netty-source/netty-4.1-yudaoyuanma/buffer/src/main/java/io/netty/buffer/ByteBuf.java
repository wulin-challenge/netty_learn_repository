...
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
     ...
     * of the new slice (= {@code length}).
     * 
     * <p> 返回从当前readerIndex开始的该缓冲区的子区域的新保留切片，并将readerIndex增加新切片的大小（=length）。
     * 
     * <p>
     ...
     * a buffer implementation that produces less garbage.
     * 
     * <p> 请注意，此方法返回的保留缓冲区与readSlice（int）不同。 此方法的行为与readSlice（...）。retain（）相似，不同之处在于此方法可能返回产生较少垃圾的缓冲区实现。
     *
     * @param length the size of the new slice
     * 
     * <p> 新切片的大小
     ...
     * 
     * <p> 在此缓冲区中将当前的readerIndex增加指定的长度。
     *
     * @throws IndexOutOfBoundsException
     *         if {@code length} is greater than {@code this.readableBytes}
     */
     ...
     * by the number of the transferred bytes (= {@code length}).
     * 
     * <p> 从当前writer索引开始将指定源缓冲区的数据传输到此缓冲区，并通过传输的字节数（=长度）增加writerIndex。
     *
     * @param srcIndex the first index of the source
     * @param length   the number of bytes to transfer
     *
     ...
     * 
     * <p> 使用指定的处理器以升序循环访问此缓冲区的指定区域。 （即索引，（索引+ 1），..（索引+长度-1））
     *
     ...
     * this buffer.
     * 
     * <p> 返回此缓冲区的子区域的一部分。 修改返回的缓冲区或此缓冲区的内容会影响彼此的内容，
     * 同时它们将维护单独的索引和标记。 此方法不会修改此缓冲区的readerIndex或writerIndex。
     * 
     * <p>
     * Also be aware that this method will NOT call {@link #retain()} and so the
     * reference count will NOT be increased.
     * 
     * <p> 另请注意，此方法将不会调用keep（），因此不会增加引用计数。
     */
     ...
     * this buffer.
     * 
     * <p> 返回此缓冲区的子区域的保留切片。 修改返回的缓冲区或此缓冲区的内容会影响彼此的内容，
     * 同时它们将维护单独的索引和标记。 此方法不会修改此缓冲区的readerIndex或writerIndex。
     * 
     * <p>
     ...
     * a buffer implementation that produces less garbage.
     * 
     * <p> 请注意，此方法返回的保留缓冲区与slice（int，int）不同。 此方法的行为类似于
     * {@code slice(...).retain()}，除了此方法可能返回产生较少垃圾的缓冲区实现之外。
     * 
     */
