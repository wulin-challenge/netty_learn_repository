 ...
 * 
 * <p> 一个ChannelHandler，它增加了对异步写入大数据流的支持，既不花费大量内存，也不会出现OutOfMemoryError。 
 * 诸如文件传输之类的大数据流需要在ChannelHandler实现中进行复杂的状态管理。 
 * ChunkedWriteHandler管理如此复杂的状态，以便您可以轻松发送大型数据流。
 * 
 * <p>
 * To use {@link ChunkedWriteHandler} in your application, you have to insert
 * a new {@link ChunkedWriteHandler} instance:
 * 
 * <p> 要在您的应用程序中使用ChunkedWriteHandler，您必须插入一个新的ChunkedWriteHandler实例：
 * 
 * <pre>
 ...
 * 
 * <p> 插入后，您可以编写ChunkedInput，以便ChunkedWriteHandler可以将其拾取并逐块获取流块的内容，
 * 并将获取的块写入下游：
 * 
 * <pre>
 * {@link Channel} ch = ...;
 ...
 * <p> 发送间歇性生成块的流
 *
 * <p> Some {@link ChunkedInput} generates a chunk on a certain event or timing.
 ...
 * 
 * <p> 一些ChunkedInput在某个事件或时间上生成一个块。 这种ChunkedInput实现通常在
 * ChunkedInput.readChunk（ChannelHandlerContext）上返回null，从而导致无限期暂停传输。 
 * 要在新块可用时继续传输，您必须调用resumeTransfer（）。
 */
