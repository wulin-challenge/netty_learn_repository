 ...
 * 
 * <p> ChannelHandler的列表，用于处理或拦截Channel的入站事件和出站操作。 ChannelPipeline实现了Intercepting Filter
 * 模式的高级形式，使用户可以完全控制事件的处理方式以及管道中的ChannelHandlers如何进行交互。
 *
 * <h3>Creation of a pipeline</h3>
 * <h3>建立管道</h3>
 *
 * <p> Each channel has its own pipeline and it is created automatically when a new channel is created.
 * 
 * <p> 每个通道都有其自己的管道，并且在创建新通道时会自动创建它。
 *
 * <h3>How an event flows in a pipeline</h3>
 * <h3>事件如何在管道中流动</h3>
 *
 * <p> The following diagram describes how I/O events are processed by {@link ChannelHandler}s in a {@link ChannelPipeline}
 * typically. An I/O event is handled by either a {@link ChannelInboundHandler} or a {@link ChannelOutboundHandler}
 * and be forwarded to its closest handler by calling the event propagation methods defined in
 * {@link ChannelHandlerContext}, such as {@link ChannelHandlerContext#fireChannelRead(Object)} and
 * {@link ChannelHandlerContext#write(Object)}.
 * 
 * <p> 下图描述了ChannelPipeline中的ChannelHandler通常如何处理I / O事件。 I / O事件由ChannelInboundHandler或
 * ChannelOutboundHandler处理，并通过调用ChannelHandlerContext中定义的事件传播方法（例如
 * ChannelHandlerContext.fireChannelRead（Object）和ChannelHandlerContext.write（Object））转发到其最近的处理程序。
 *
 * <pre>
 *                                                 I/O Request
 *                                            via {@link Channel} or
 ...
 * 
 * <p> 入站事件由入站处理程序按自下而上的方向进行处理，如图中左侧所示。 入站处理程序通常会处理由图底部的I / O线程生成的入站数据。 
 * 通常通过实际的输入操作（例如SocketChannel.read（ByteBuffer））从远程对等方读取入站数据。 如果入站事件超出了顶部入站处理程序的范围，
 * 则将其静默丢弃，或者在需要引起注意时记录下来。
 * 
 * <p>
 ...
 * 
 * <p> 出站事件由出站处理程序按自顶向下的方向进行处理，如图中右侧所示。 出站处理程序通常会生成或转换出站流量，例如写请求。 
 * 如果出站事件超出了底部出站处理程序，则由与通道关联的I / O线程处理。 I / O线程通常执行实际的输出操作，例如SocketChannel.write（ByteBuffer）。
 * 
 * <p>
 ...
 * <p>
 * 
 * <p> 在上面的示例中，其名称以Inbound开头的类表示它是一个入站处理程序。 名称以Outbound开头的类表示它是一个出站处理程序。
 * 
 ...
 * 
 * <p> 在给定的示例配置中，事件进入时，处理程序评估顺序为1、2、3、4、5。 当事件出站时，顺序为5、4、3、2、1。
 * 基于此原理，ChannelPipeline跳过对某些处理程序的求值以缩短堆栈深度：
 * 
 * <ul>
 ...
 * <li></li>
 * <li>如果5同时实现ChannelInboundHandler和ChannelOutboundHandler，则入站和出站事件的评估顺序可能分别为125和543。</li>
 * </ul>
 *
 ...
 * 
 * <p> 如您在图中所示，您可能会注意到，处理程序必须调用ChannelHandlerContext中的事件传播方法，以将事件转发到其下一个处理程序。 
 * 这些方法包括：
 * <ul>
 ...
 * </ul>
 *
 * <p> and the following example shows how the event propagation is usually done:
 * 
 * <p> 以下示例说明了事件传播通常是如何完成的：
 *
 * <pre>
 ...
 * 
 * <p> 假定用户在管道中具有一个或多个ChannelHandler，以接收I / O事件（例如，读取）并请求I / O操作（例如，写入和关闭）。 
 ...
 * <li> </li>
 * <li>Protocol Decoder-将二进制数据（例如ByteBuf）转换为Java对象。</li>
 * <li> </li>
 ...
 * 
 * <p> 告诉管道在与I / O线程不同的线程中运行MyBusinessLogicHandler的事件处理程序方法，以使I / O线程不会被耗时的任务阻塞。
 * 
 * // If your business logic is fully asynchronous or finished very quickly, you don't
 * // need to specify a group.
 * 
 * <p> 如果您的业务逻辑是完全异步的或很快完成的，则无需指定组。
 * 
 ...
 * after the exchange.
 * 
 * <p> 由于ChannelPipeline是线程安全的，因此可以随时添加或删除ChannelHandler。 例如，您可以在即将交换敏感信息时插入加密处理程序，并在交换后将其删除。
 */
