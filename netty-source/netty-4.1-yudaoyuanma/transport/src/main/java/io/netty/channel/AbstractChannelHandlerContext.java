...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap
     ...
     * 
     * <p> 尽最大努力检测是否已调用ChannelHandler.handlerAdded（ChannelHandlerContext）。 
     * 如果不是，则返回false；如果未调用，则返回true。
     *
     * <p> If this method returns {@code false} we will not invoke the {@link ChannelHandler} but just forward the event.
     * This is needed as {@link DefaultChannelPipeline} may already put the {@link ChannelHandler} in the linked-list
     * but not called {@link ChannelHandler#handlerAdded(ChannelHandlerContext)}.
     * 
     * <p> 如果此方法返回false，我们将不调用ChannelHandler，而只是转发事件。 这是必需的，因为DefaultChannelPipeline
     * 可能已将ChannelHandler放在链接列表中，但未称为ChannelHandler.handlerAdded（ChannelHandlerContext）。
     * 
     */
