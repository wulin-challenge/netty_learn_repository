...
public interface ChannelConfig {
     ...
     * 
     * <p> 返回写操作的最大循环计数，直到WritableByteChannel.write（ByteBuffer）返回非零值为止。 
     * 它与并发编程中使用的自旋锁相似。 根据JVM运行的平台，它可以提高内存利用率和写入吞吐量。 预设值为16。
     */
     ...
     * the platform that JVM runs on.  The default value is {@code 16}.
     * 
     * <p> 设置写操作的最大循环数，直到WritableByteChannel.write（ByteBuffer）返回非零值为止。 
     * 它与并发编程中使用的自旋锁相似。 根据JVM运行的平台，它可以提高内存利用率和写入吞吐量。 预设值为16。
     *
