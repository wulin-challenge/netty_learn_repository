...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
     ...
     * 
     * <p> 创建一个新队列，其中将包含要执行的任务。 此默认实现将返回LinkedBlockingQueue，但如果SingleThreadEventExecutor的子类将不对此
     * Queue进行任何阻塞调用，则@Override this可能有意义，并返回一些完全不支持阻塞操作的高性能实现。
     */
     ...
     * 
     * <p> 从任务队列中轮询所有任务，然后通过Runnable.run（）方法运行它们。
     *
     * @return {@code true} if and only if at least one task was run
     * 
     * <p> 当且仅当至少一项任务已运行时，才为true
     */
     ...
     * 
     * <p> 从任务队列中轮询所有任务，然后通过Runnable.run（）方法运行它们。 此方法停止运行任务队列中的任务，如果运行时间超过timeoutNanos，则返回该方法。
     *
     * @return 是否有执行到任务
     */
     ...
     * 
     * <p> 从runAllTasks（）和runAllTasks（long）返回之前调用。
     */
    @UnstableApi
     ...
     * 
     * <p> 返回直到执行具有最接近截止时间的调度任务的剩余时间。(即距离指定时间，还要多久可执行)
     */
     ...
     * it is fully started.
     * 
     * <p> 返回为SingleThreadEventExecutor供电的线程的ThreadProperties。 如果SingleThreadEventExecutor尚未启动，则此操作将启动它并阻塞直到它完全启动。
     */
